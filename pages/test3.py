...
var_type = st.radio('Which variety will you grow for the upcoming season?',
                                 ['Inbred', 'Hybrid'])

growth_ops = st.selectbox('What is the growth duration of the variety you grew '
                          "in the last season with the same sowing time?",
                          ["91-100", "101-110","111-120", "121-130"])

crops = st.selectbox("What was grown in the last three months before the "
                     "upcoming rice?", ['Rice', 'Fallow', 'Legumes'])

organic = st.number_input("In this upcoming season, will you apply "
                          "organic fertilizer?"
                          , value=0)

rate = st.number_input("What is th approximate seed rate?", value=0)

sources = ['Purchased certified seeds with tag or provided by DA',
         'Purchased or exchanged seeds from other farmer using good quality seed or from a farm lot with proper rouging',
         'Home-saved seeds or seeds used for several seasons',
         'Seeds from the first harvest of certified seeds with tag']
source_up = st.selectbox("Where will you get the seeds for this "
                           "UPCOMING season?",
                            sources)
source_prev = st.selectbox("Where did you get the seeds for this "
                         "PREVIOUS season?", sources)
